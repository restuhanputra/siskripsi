...
class Auth extends CI_Controller
{
	...
	function reset()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			// post data
			$npm = blockSQLInjection($this->input->post('username'));

			// get data
			$query = $this->user->get($npm);
			if ($query->num_rows() > 0) {
				$mhs = $query->row_array();
				$email = $mhs['email'];
			}
			...
			if ($this->db->affected_rows() > 0) {
				...
				$config = array(
					...
					'smtp_pass' => '', //password email untuk mengirim password baru
					'mailtype'  => 'html',
					'charset'   => 'iso-8859-1'
				);
