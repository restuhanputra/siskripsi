			...
			<div class="box" style="border-radius: none; border-top: none;">
				...
				<div class='box-header with-border'>
					...
					<h3 class='box-title pull-right'>
						<a href="<?= site_url('proposal') ?>" class="btn btn-sm btn-flat btn-warning"><i class='fa fa-undo'></i> KEMBALI</a>
					</h3>
				</div><!-- /.box-header -->
				...
				<form class="form" enctype="multipart/form-data" action="" method="post">
					<div class="box-body">
						...
						<input type="hidden" name="proposal_id" value="<?= $g_proposal_id['proposal_id'] ?>">
						...
						<div class="form-group row <?= form_error('nama_lkp') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" name="nama_lkp" placeholder="Nama Lengkap" value="<?= $g_mhs_id['nama_lkp'] ?>" readonly>
								<?= form_error('nama_lkp') ?>
							</div>
						</div>
						<div class="form-group row <?= form_error('npm') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<input type="number" class="form-control" name="npm" placeholder="NPM" value="<?= $g_mhs_id['npm'] ?>" readonly>
								<?= form_error('npm') ?>
							</div>
						</div>
						<div class="form-group row <?= form_error('prodi') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" name="prodi" placeholder="Program Studi" value="<?= $g_mhs_id['prodi_nama'] ?>" readonly>
								<?= form_error('prodi') ?>

								<input type="hidden" name="prodi_id" value="<?= $g_mhs_id['prodi_id'] ?>">
							</div>
						</div>
						<div class="form-group row <?= form_error('kelas') ? 'has-error' : null ?>">
							<label for="kelas" class="col-sm-4 control-label">Kelas *</label>

							<div class="col-sm-8">
								<input type="text" class="form-control" name="kelas" placeholder="Kelas" value="<?= $g_proposal_id['kelas'] ?>">
								<?= form_error('kelas') ?>
							</div>
						</div>
						<div class="form-group row <?= form_error('no_telp') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<input type="number" class="form-control" name="no_telp" placeholder="No. Telp" value="<?= $g_proposal_id['no_telp'] ?>">
								<?= form_error('no_telp') ?>
							</div>
						</div>
						<div class="form-group row <?= form_error('email') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<input type="email" class="form-control" name="email" placeholder="E-mail" value="<?= $g_proposal_id['email'] ?>">
								<?= form_error('email') ?>
							</div>
						</div>
						<div class="form-group row <?= form_error('judul_proposal') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<textarea class="form-control" rows="5" name="judul_proposal" placeholder="Judul Proposal Skripsi"><?= $g_proposal_id['judul_proposal'] ?></textarea>
								<?= form_error('judul_proposal') ?>
							</div>
						</div>
						<div class="form-group row <?= form_error('file_proposal') ? 'has-error' : null ?>">
							<label for="file_proposal" class="col-sm-4 control-label">File Proposal Skripsi</label>
			...
			<div class="box" style="border-radius: none; border-top: none;">
				...
				<div class="box-body">
					<ol style="padding-inline-start: 20px !important;">
						...
						<li>File Proposal Skripsi harus ber-ekstensi (<i>.pdf</i> ).</li>
						<li>Size (ukuran) file Proposal Skripsi maximal 10 MB.</li>
					</ol>
				</div> <!-- /.box-body -->
			</div> <!-- /.box -->
