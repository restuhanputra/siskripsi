			...
			<div class="box">
				...
				<div class="box-body">
					<form class="form" enctype="multipart/form-data" action="" method="post">
						...
						<input type="hidden" name="sdg_skripsi_id" value="<?= $g_jdw_skripsi_id['sdg_skripsi_id'] ?>">
						<?php
						$smt = $g_jdw_skripsi_id['semester'] == 1 ? "GANJIL" : "GENAP"  ?>
						<div class="form-group row <?= form_error('semester') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" name="semester" placeholder="Semester" value="<?= $g_jdw_skripsi_id['tahun'] . ' ' . $smt ?>" readonly>
								<?= form_error('semester') ?>
							</div>
						</div>
						...
						<div class="form-group row <?= form_error('npm') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" name="npm" placeholder="NPM" value="<?= $g_jdw_skripsi_id['npm'] ?>" readonly>
								<?= form_error('npm') ?>
							</div>
						</div>
						...
						<div class="form-group row <?= form_error('nama_lkp') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" name="nama_lkp" id="nama_lkp" placeholder="Nama Lengkap" value="<?= $g_jdw_skripsi_id['nama_lkp'] ?>" readonly>
								<?= form_error('nama_lkp') ?>
							</div>
						</div>
						...
						<div class="form-group row <?= form_error('judul_skripsi') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<textarea class="form-control" rows="5" name="judul_skripsi" id="judul_skripsi" placeholder="Judul Skripsi" readonly><?= $g_jdw_skripsi_id['judul_skripsi'] ?></textarea>
						...
						<div class="form-group row <?= form_error('dpj_satu') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<?php
								if (!empty($g_jdw_skripsi_id['dpj_satu'])) {
									foreach ($g_dosen as $data) :
										$nidn     = $data['nidn'];
										$nama_dsn = strtoupper($data['nama_lkp']);

										if ($g_jdw_skripsi_id['dpj_satu'] == $nidn) {
								?>
						...
						<div class="form-group row <?= form_error('dpj_dua') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<?php
								if (!empty($g_jdw_skripsi_id['dpj_dua'])) {
									foreach ($g_dosen as $data) :
										$nidn     = $data['nidn'];
										$nama_dsn = strtoupper($data['nama_lkp']);

										if ($g_jdw_skripsi_id['dpj_dua'] == $nidn) {
								?>
						...
						<div class="form-group row <?= form_error('dpj_tiga') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<?php
								if (!empty($g_jdw_skripsi_id['dpj_tiga'])) {
									foreach ($g_dosen as $data) :
										$nidn     = $data['nidn'];
										$nama_dsn = strtoupper($data['nama_lkp']);

										if ($g_jdw_skripsi_id['dpj_tiga'] == $nidn) {
								?>
											<input type="text" class="form-control" name="dpj_tiga" placeholder="Dosen Penguji III" value="<?= $nama_dsn ?>" readonly>
									<?php
										}
									endforeach;
								} else {
									?>
									<input type="text" class="form-control" name="dpj_tiga" placeholder="Dosen Penguji III" readonly>
								<?php
								}
								?>
								<?= form_error('dpj_tiga') ?>
						...
						<div class="form-group row <?= form_error('tanggal_sdg') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<?php
								if (!empty($g_jdw_skripsi_id['tanggal_sdg'])) {
								?>
									<input type="text" class="form-control pull-right" name="tanggal_sdg" placeholder="Tanggal Sidang" id="datepicker" value="<?= $g_jdw_skripsi_id['tanggal_sdg'] ?>" autocomplete="off">
								<?php
								} else {
								?>
						...
						<div class="form-group row <?= form_error('waktu_id') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<select class="form-control" name="waktu_id">
									<?php
									if (!empty($g_jdw_skripsi_id['waktu_id'])) {
										foreach ($g_waktu as $data) :
											$waktu_id = $data['waktu_id'];
											$jam      = strtoupper($data['jam']);

											if ($g_jdw_skripsi_id['waktu_id'] == $waktu_id) {
									?>
						...
						<div class="form-group row <?= form_error('ruangan') ? 'has-error' : null ?>">
							...
							<div class="col-sm-8">
								<?php
								if (!empty($g_jdw_skripsi_id['ruangan'])) {
								?>
									<input type="text" class="form-control" placeholder="Ruangan" name="ruangan" value="<?= $g_jdw_skripsi_id['ruangan'] ?>">
								<?php
								} else {
								?>
