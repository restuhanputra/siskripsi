<section class="content">
	<!-- Small boxes (Stat box) -->
	<div class="row">
		<div class="col-xs-8">
			<div class="box">

				<div class='box-header with-border'>
					<h3 class='box-title pull-right'>
						<a href="<?= site_url('tahun_akademik') ?>" class="btn btn-sm btn-flat btn-warning"><i class='fa fa-undo'></i> Kembali</a>
					</h3>
				</div><!-- /.box-header -->

				<form class="form" enctype="multipart/form-data" action="" method="post">
					<div class="box-body">

						<input type="hidden" name="tahun_akademik_id" value="<?= $thn_akdmk_id['tahun_akademik_id'] ?>">

						<div class="form-group row <?= form_error('tahun') ? 'has-error' : null ?>">
							<label for="tahun" class="col-sm-4 control-label">Tahun *</label>

							<div class="col-sm-8">
								<input type="text" class="form-control" name="tahun" placeholder="Tahun" value="<?= $thn_akdmk_id['tahun'] ?>">
								<?= form_error('tahun') ?>
							</div>
						</div>

						<div class="form-group row <?= form_error('semester') ? 'has-error' : null ?>">
							<label for="semester" class="col-sm-4 control-label">Semester *</label>

							<div class="col-sm-8">
								<select class="form-control" name="semester">
									<?php if ($thn_akdmk_id['semester'] == "1") {
										echo '<option value="1" selected>GANJIL</option>';
										echo '<option value="2">GENAP</option>';
									} else {
										echo '<option value="2" selected>GENAP</option>';
										echo '<option value="1">GANJIL</option>';
									}
									?>
								</select>
								<?= form_error('semester') ?>
							</div>
						</div>

						<div class="form-group row <?= form_error('aktifkah') ? 'has-error' : null ?>">
							<label for="aktifkah" class="col-sm-4 control-label">Status *</label>

							<div class="col-sm-8">
								<select class="form-control" name="aktifkah">
									<?php if ($thn_akdmk_id['aktifkah'] == "2") {
										echo '<option value="2" selected>AKTIF</option>';
										echo '<option value="1">TIDAK AKTIF</option>';
									} else {
										echo '<option value="1" selected>TIDAK AKTIF</option>';
										echo '<option value="2">AKTIF</option>';
									}
									?>
								</select>
								<?= form_error('aktifkah') ?>
							</div>
						</div>

						<div class="box-footer text-center">
							<button type="reset" class="btn btn-flat btn-default">Reset</button>
							&nbsp;
							<button type="submit" name="edit" class="btn btn-flat btn-success"><i class="fa fa-paper-plane"></i> Edit</button>
						</div> <!-- /.box-footer -->

					</div> <!-- /.box-body -->

				</form> <!-- /.form -->

			</div> <!-- /.box -->
		</div> <!-- /.col-xs-8 -->

		<div class="col-xs-4">

			<div class="box">
				<div class="box-header with-border">
					<i class="fa fa-bullhorn"></i>
					<h3 class="box-title text-bold">CATATAN</h3>
				</div>
				<!-- /.box-header -->
				<div class="box-body">
					<ol style="padding-inline-start: 20px !important;">
						<li>Setiap field yang bertanda <b>*</b> wajib diisi.</li>
					</ol>
				</div> <!-- /.box-body -->
			</div> <!-- /.box -->

		</div> <!-- /.col-xs-4 -->

	</div> <!-- /.row -->

</section> <!-- /.section -->
