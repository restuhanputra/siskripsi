		...
		<div class="col-xs-12">
			...
			<div class="box">
				...
				<div class='box-header with-border'>
					<h3 class='box-title'>
						<form action="<?= site_url('wisuda/excel') ?>" method="post" target="_blank">
							<input type="hidden" name="tahun_akademik_id" value="<?= $g_tahun_akademik_id ?>">

							<button onclick="return confirm('Download data (.xlsx), Apakah anda yakin ?')" class="btn btn-sm btn-flat btn-success"> <i class="fa fa-file-excel-o"></i> EXCEL</button>
						</form>
					</h3>
							...
							<tbody>
								...
								if (!empty($g_wisuda)) {
									...
									foreach ($g_wisuda as $data) :
										...
										$judul_skripsi = $data['judul_skripsi_ina'];
										$revisi        = $data['revisi'];
								...
								?>
										...
										<tr>
											...
											<td class="text-center">
												...
												?>
													<form action="<?= site_url('wisuda/revisi') ?>" method="post">
														...
														<button onclick="return confirm('Revisi Data, Apakah anda yakin ?')" href="<?= site_url('wisuda/revisi') ?>" class="btn btn-sm btn-flat btn-warning"><i class="fa fa-edit"></i></button>
													</form>
												<?php
												} else {
												?>
											...
											<td class="text-center">
												<form action="<?= site_url('wisuda/delete') ?>" method="post">
													<input type="hidden" name="wisuda_id" value="<?= $wisuda_id ?>">

													<button onclick="return confirm('Delete data, Apakah anda yakin ?')" href="<?= site_url('wisuda/delete') ?>" class="btn btn-sm btn-flat btn-danger"> <i class="fa fa-trash"></i></button>
												</form>
											</td>
										</tr>
										<!-- /. Isi Tabel -->
