...
class Sdg_proposal_m extends CI_Model
{
	...
	// donut chart
	function count_sdg_proposal($tahun_akademik_id, $status)
	{
		$this->db->from($this->sdg_proposal);
	...
	{
		$this->db->select('sdg_proposal.*, semester as semester, tahun as tahun, prodi.nama as prodi_nama');
		$this->db->from($this->sdg_proposal)
			->join($this->mahasiswa, 'mahasiswa.npm = sdg_proposal.npm')
			->join($this->prodi, 'prodi.prodi_id = sdg_proposal.prodi_id')
			->join($this->tahun_akademik, 'tahun_akademik.tahun_akademik_id = sdg_proposal.tahun_akademik_id');
	...
	{
		$this->db->select('sdg_proposal.*, semester as semester, tahun as tahun, bimbingan.pbb_satu as pbb_satu, bimbingan.pbb_dua as pbb_dua');
		$this->db->from($this->sdg_proposal)
			->join($this->mahasiswa, 'mahasiswa.npm = sdg_proposal.npm')
			->join($this->prodi, 'prodi.prodi_id = sdg_proposal.prodi_id')
			->join($this->tahun_akademik, 'tahun_akademik.tahun_akademik_id = sdg_proposal.tahun_akademik_id')
			->join($this->bimbingan, 'bimbingan.proposal_id = sdg_proposal.proposal_id');
	...
	function revisi($sdg_proposal_id)
	{
		$params = array(
			'revisi'  => 1,
	...
	function delete($sdg_proposal_id)
	{
		$this->db->where('sdg_proposal_id', $sdg_proposal_id)
			->delete($this->sdg_proposal);
	}
} /* /.class */
